...
runners.forEach(function(cv) {
    return fullName.push(cv.first_name + " " + cv.last_name);
});
...
let allCaps = [];
runners.map(function(cv) {
    return allCaps.push((cv.first_name).toUpperCase());
});
console.log(allCaps); 
...
let largeShirts = [];
runners.filter(function(cv) {
    if (cv.shirt_size === "L") {
        return largeShirts.push(cv);
    } else {
...
ticketPriceTotal.push(runners.reduce(function(added, cv) {
    return added + cv.donation;
}, 0));
...
// Problem 1
// Use .filter() to get a list of runners' full names and id# with donations that exceed 200.
let topdonators = [];
runners.filter(function(cv) {
    if (cv.donation > 200) {
        return topdonators.push(cv.id + " " + cv.first_name + " " + cv.last_name);
    } else {
...
// Problem 2
// Use .forEach() to create a new array that contains each runner's first name-company name.
let companywars = [];
runners.forEach(function(cv) {
    return companywars.push(cv.first_name + "-" + cv.company_name);
});
console.log(companywars);

// Problem 3
//  Use .map() to add 999 to all runners' donation amounts
let fakedonos = [];
runners.map(function(cv) {
    return fakedonos.push(cv.donation + 999);
});
console.log(fakedonos);
